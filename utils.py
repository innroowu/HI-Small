...
def filter_sequences_by_time(in_sentences, out_sentences, in_sentences_len, out_sentences_len, 
                             time_threshold, sort_dim=-1, max_length=32):
    """
    根據時間閾值過濾交易序列，只保留 t < time_threshold 的交易
    
    Args:
        in_sentences: 所有入帳序列 [num_nodes, max_len, num_features]
        out_sentences: 所有出帳序列 [num_nodes, max_len, num_features]
        in_sentences_len: 入帳序列長度
        out_sentences_len: 出帳序列長度
        time_threshold: 時間閾值（只保留小於此值的交易）
        sort_dim: 時間戳在特徵中的維度（預設 -1，即最後一維）
        max_length: 最大序列長度
    
    Returns:
        filtered_in_sentences: 過濾後的入帳序列
        filtered_out_sentences: 過濾後的出帳序列
        filtered_in_lens: 過濾後的入帳序列長度
        filtered_out_lens: 過濾後的出帳序列長度
    """
    num_nodes = in_sentences.shape[0]
    ...
    for node_id in range(num_nodes):
        # 處理入帳序列
        in_seq = in_sentences[node_id][:in_sentences_len[node_id]]  # 取有效長度
        in_times = in_seq[:, sort_dim]  # 取時間戳
        in_mask = in_times < time_threshold  # 只保留 t < threshold 的交易
        filtered_in = in_seq[in_mask]
        
        # 限制最大長度
        if len(filtered_in) > max_length:
            # 按時間排序，取最近的 max_length 筆
            sorted_indices = torch.argsort(filtered_in[:, sort_dim], descending=True)[:max_length]
            sorted_indices = torch.sort(sorted_indices).values  # 恢復時間順序
            filtered_in = filtered_in[sorted_indices]
        
        filtered_in_list.append(filtered_in)
        filtered_in_lens.append(len(filtered_in))
        
        # 處理出帳序列
        out_seq = out_sentences[node_id][:out_sentences_len[node_id]]
        out_times = out_seq[:, sort_dim]
        out_mask = out_times < time_threshold
        filtered_out = out_seq[out_mask]
        
        if len(filtered_out) > max_length:
            sorted_indices = torch.argsort(filtered_out[:, sort_dim], descending=True)[:max_length]
            sorted_indices = torch.sort(sorted_indices).values
            filtered_out = filtered_out[sorted_indices]
        
        filtered_out_list.append(filtered_out)
        filtered_out_lens.append(len(filtered_out))
    
    # Pad sequences
    filtered_in_sentences = pad_sequence(filtered_in_list, batch_first=True, padding_value=0.0)
    filtered_out_sentences = pad_sequence(filtered_out_list, batch_first=True, padding_value=0.0)
    filtered_in_lens = torch.tensor(filtered_in_lens, dtype=torch.long)
    filtered_out_lens = torch.tensor(filtered_out_lens, dtype=torch.long)
    
    return filtered_in_sentences, filtered_out_sentences, filtered_in_lens, filtered_out_lens
