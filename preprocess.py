...
def generate_sequences(args):
    ...
    # 🔧 修改為：明確指定 timestamp 在第 4 維（index 4）
    start_time = edge_attr[:, 4].min().item()  # timestamp 固定在第 4 維
    edge_attr[:, 4] = edge_attr[:, 4] - start_time  # 只修改 timestamp 欄位
...
if __name__ == '__main__':
    ...
    argparser.add_argument('--sort-by', type=int, default=4, #noGFP = -1
